...
enum FLAGS { INVULNERABLE, ROOTED, UNTARGETABLE, EVASIVE, UNINTERRUPTIBLE, IMMOVABLE, STUNNED, INVISIBLE, CRITICAL }
...
func set_critical(val):
    flags[FLAGS.CRITICAL] = flags[FLAGS.CRITICAL] + val

func has_critical():
    return flags[FLAGS.CRITICAL]>0